...
const RSHandle = styled(({ isDragging, isVisible, pos, ...props }) => <div {...props} />)`
...
class Instance extends Component {
  ...
  setNewTime(e) {
    ...
    this.setState(prevState => ({
      [dragging]: newTime >= RANGE_MIN && newTime <= RANGE_MAX ? newTime : prevState[dragging],
    }));
  ...
  moveHandle(edge, dir) {
    ...
    const calcVal = prevState => {
      if (dir === 'fwd') {
        return prevState[edge] + UNIT < RANGE_MAX ? prevState[edge] + UNIT : RANGE_MAX;
      }
      if (dir === 'bwd') {
        return prevState[edge] - UNIT > RANGE_MIN ? prevState[edge] - UNIT : RANGE_MIN;
      }
      return null;
    };
    this.setState(
    ...
    return (
      <>
          ...
          {popupState => (
            <>
              <RSInstance
                ...
                {!isLocked ? <div {...bindHover(popupState)} style={{ width: `100%`, height: `28px` }} /> : null}
              </RSInstance>
              {!this.state.dragging ? (
                <InstancePopover
        ...
        {handles.map(handle => {
          ...
          const isHandleActive = this.state.dragging === edge || this.state.overHandle === edge;
          return (
            <PopupState key={`${edge}Popover`} popupId={`${edge}Popover`} variant="popover">
              {popupState => (
                <>
                  <RSHandle
                    isDragging={this.state.dragging === edge}
                    isVisible={isHandleActive || (this.state.overInstance && !this.state.dragging) || popupState.isOpen}
                    ...
                    <div
                      ...
                      {isHandleActive ? <MeTooltip isVisible>{formatSeconds(value)}</MeTooltip> : null}
                    </div>
                  </RSHandle>
                  {!this.state.dragging ? (
                    <HandlePopover
